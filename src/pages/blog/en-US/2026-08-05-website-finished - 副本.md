...
The on-site search uses Pagefind, but by default it tokenizes by English words and basically can't handle Chinese — God knows how it splits the text, so you might search a keyword and find absolutely nothing. For example, searching 阿拉 would actually surface 阿拉伯 (Arab), but searching 阿拉伯 itself would come up empty; searching 中国 (China) would return 中日文 (Chinese & Japanese), and happened to drag 一中 (No.1 High School) along for the ride, but never 中国 itself.
